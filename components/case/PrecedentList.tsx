...
import { copyCitationToClipboard } from "@/lib/export";
...
export function PrecedentList({
  ...
  async function handleCopy(index: number, source: string) {
    try {
      await copyCitationToClipboard(source);
  ...
  return (
    <div className="flex flex-col gap-3">
      {precedents.map((precedent, index) => {
        ...
        const source = meta?.source ?? `Authority ${index + 1}`;
          ...
          <div key={index} className="border border-border bg-card">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-3.5 py-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => void handleCopy(index, source)}
                  aria-label={`Copy citation ${source}`}
                  className="inline-flex h-6 w-6 items-center justify-center border border-border text-muted-foreground transition-colors duration-150 hover:border-border-strong hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-stone-900"
                >
                  {isCopied ? (
